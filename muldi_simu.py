...
targetPos = ti.Vector.field(2, dtype=ti.f32, shape=[TARGET_N])
...
def init():
    for p in ti.grouped(grid):
        grid[p] = 0.0
    for i in position:
        position[i] = ti.Vector([2*ti.random()-1, 2*ti.random()-1]) * INIT_SIZE + ti.Vector([GRID_SIZE/2, GRID_SIZE/2])
        heading[i] = ti.random() * np.pi * 2.0
    for i in targetPos:
        targetPos[i] = ti.Vector([ti.random(), ti.random()]) * SEARCH_SIZE
...
def step(phase: ti.i32):
    ...
    # stimuli
    for i in targetPos:
        ipos = targetPos[i].cast(int) % GRID_SIZE
        grid[phase, ipos] += 1.0
