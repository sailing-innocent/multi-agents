...
@ti.kernel
def emergency(i: ti.i32):
  if i > 500:
    # WHEN EMERGENCE OCCURS
    MOVE_STEP = 0.8
    MOVE_ANGLE = 0.2 * np.pi * 0.8
    SENSE_ANGLE = 0.4 * np.pi * 0.8
    SENSE_DIST =  4
    EVAPORATION = 0.9
...
folder = "./out/emergency"
if not os.path.isdir(folder):
  os.mkdir(folder)

STOP = False 
while gui.running and not gui.get_event(gui.ESCAPE) and not STOP:
  # check if stop
  ...
  # gui.circles(InterestPos/GRID_SIZE, radius=5, color=0xd0104c)
  gui.show()